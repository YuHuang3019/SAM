...
if(.not.SFC_FLX_FXD) then
  ...
  if(OCEAN.or.LAND.and.SLM) then
    ...
    if(LES) then
       ...
       q_s = salt_factor*qsatw(sstxy(1,1)+t00,presi(1))
       ...
       do j=1,ny
         do i=1,nx
          ...
          if(landmask(i,j).eq.0) then
           ...
           u_h = 0.5*(u(i+1,j,1)+u(i,j,1)) 
           v_h = 0.5*(v(i,j+YES3D,1)+v(i,j,1)) 
           ta_h = tabs(i,j,1)
           q_h = qv(i,j,1)
  ...
  if(LAND) then
     ...
     if(LES) then
           ...
           v_h = v0(1)
           q_h = qv0(1)
           t_h = tabs0(1)*prespot(1)
           t_s = (sstxy(1,1)+t00)*prespoti(1)
           q_s = soil_wetness*qsatw(sstxy(1,1)+t00,presi(1))
       ...
       if(SLM) then
          ...
          do j=1,ny
           do i=1,nx
             zr(i,j) = z(1)
             presr(i,j) = pres(1)
             ur(i,j) = 0.5*(u(i+1,j,1)+u(i,j,1)) 
             vr(i,j) = 0.5*(v(i,j+YES3D,1)+v(i,j,1)) 
             tr(i,j) = tabs(i,j,1)
             qvr(i,j) = qv(i,j,1)
